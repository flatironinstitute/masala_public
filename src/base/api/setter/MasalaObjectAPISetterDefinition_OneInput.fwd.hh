...
namespace setter {
	...
	template< typename T1 >
	using MasalaObjectAPISetterDefinition_OneInputSP = MASALA_SHARED_POINTER< MasalaObjectAPISetterDefinition_OneInput< T1 > >;

	/// @brief We will use the convention that an class name followed by CSP
	/// represents a MASALA_SHARED_POINTER for const objects of that class.
	template< typename T1 >
	using MasalaObjectAPISetterDefinition_OneInputCSP = MASALA_SHARED_POINTER< MasalaObjectAPISetterDefinition_OneInput< T1 > const >;

	/// @brief We will use the convention that an class name followed by WP
	/// represents a MASALA_WEAK_POINTER for objects of that class.
	template< typename T1 >
	using MasalaObjectAPISetterDefinition_OneInputWP = MASALA_WEAK_POINTER< MasalaObjectAPISetterDefinition_OneInput< T1 > >;

	/// @brief We will use the convention that an class name followed by CWP
	/// represents a MASALA_WEAK_POINTER for const objects of that class.
	template< typename T1 >
	using MasalaObjectAPISetterDefinition_OneInputCWP = MASALA_WEAK_POINTER< MasalaObjectAPISetterDefinition_OneInput< T1 > const >;
